...
class CalendarDatePicker extends StatefulWidget {
  ...
  // final ValueChanged<DateTime> dateFromButton;
    ...
    switch (_mode) {
      case DatePickerMode.day:
        return _MonthPicker(
          ...
          // dateFromButton: widget.dateFromButton,
        );
...
class _DatePickerModeToggleButtonState extends State<_DatePickerModeToggleButton> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final  start = MediaQuery.of(context).size.width*0.5;

    return 
      Padding(

        padding: const EdgeInsetsDirectional.only(start: 100, end: 0),
        child: Row(
...
extension on MaterialLocalizations {
  get shortWeekdays => _shortWeekdays;
}
...
class _Day extends StatefulWidget {
 // DateTime days = DateTime.now();
  ...
  // bool isToday =true;
  // ValueChanged<DateTime> onChanged(DateTime days) => onChanged(days);
  // FocusNode? focusNode;
    _Day(
      this.day, {
        super.key,
