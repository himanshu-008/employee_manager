...
import 'package:employment_manager/calendar_pick.dart' as cp;
...
class _DatePickerDialogState extends State<DatePickerDialog> with RestorationMixin {
  ...
  // ################################################################
  late final ValueChanged<DateTime?> _datetime ;

  void _handleChangeButton (DateTime date){
  ...
  DateTime nextWeek(DateTime date) {
    ...
    var day = DateTime(date.year, date.month, date.day + (date.weekday + (7 - date.weekday )));

    return day ;
  }
  DateTime nextMonday(DateTime date) {
    var monday = DateTime(date.year, date.month, date.day + (8-date.weekday));

   return monday ;
  }
  DateTime nextTuesday(DateTime date) {
    var tuesday = DateTime(date.year, date.month, date.day + (9-date.weekday));
  ...
  void _handleDateChanged(DateTime date) {
      ...
      // ref.watch(cp.selectDataProvider).onChanged(date);
     setState(() {
       _selectedDate.value = date;
     });
  ...
  Widget build(BuildContext context) {
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        Text(_selectedDate.value == null ? '' : formatShortDate(_selectedDate.value!),)
                      ],
                    ),
    ...
    cp.CalendarDatePicker calendarDatePicker() {
      return cp.CalendarDatePicker(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                        ...
                        onPressed: () {
                        _handleChangeButton(DateTime.now());
                      ...
                      Expanded(
                        child: ElevatedButton(
                          onPressed: () {
                            var a = nextMonday(DateTime.now());
                            _handleChangeButton(a);
                          },
                          style: const ButtonStyle(
                              // backgroundColor:
                              //     WidgetStatePropertyAll(
                  ...
                  child: Row(
                    children: [
                      Expanded(
                          child: ElevatedButton(
                              onPressed: () {
                                _handleChangeButton(nextTuesday(DateTime.now()));
                      ...
                      Expanded(
                          child: ElevatedButton(
                              onPressed: () {
                                _handleChangeButton(nextWeek(DateTime.now()));
                              },
                              style: const ButtonStyle(
                                  // backgroundColor:
                                      // WidgetStatePropertyAll(
                                ...
                                Expanded(
                                  child: ElevatedButton(
                                    onPressed: () {
                                      ...
                                      _handleChangeButton(nextMonday(DateTime.now()));
                                    },
                                    style: const ButtonStyle(
                                      // backgroundColor:
                                      //     WidgetStatePropertyAll(
